function [game_finished, full_game_matrix, new_im, move_number] = checkMove2(full_game_matrix, location_matrix, cam, move_number, side_length, prev_im)
        ...
        properties = regionprops(CC, 'Centroid', 'Area', 'MajorAxisLength');
        
        
        objects = CC.NumObjects;
        areas = [properties.Area];
        heights = [properties.MajorAxisLength];
%         if max(areas) > 50000
%             new_im = prev_im;
%             game_finished = 0;
%             return;
%         end
        ...
        min_dist = 0.75*side_length;
        index = 1;
        poi = [0,0,0];
        for i = 1:objects
            if areas(i) > (1/4)*(side_length)^2%4000
                for k = 1:8
                    for l = 1:8
                        dist = pdist([location_matrix(k,l,1), location_matrix(k,l,2); centroids(i,:)]);
                        if dist < min_dist
                            
                             if areas(i) > 3*(side_length^2)
                                new_im = prev_im;
                                game_finished = 0;
                                return;
                             end
            ...
            switch size(poi, 1)
                ...
                case 2
                    ...
                    if piece1 > 0 && piece2 == 0
                        ...
                        MovePiece([9-poi(1,1), poi(1,2)], [9-poi(2,1), poi(2,2)], piece1);
